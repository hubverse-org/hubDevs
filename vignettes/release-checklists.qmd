...
## Release Checklist
 ...
 - [ ] (For R packages) Update version number in `DESCRIPTION` and check that the `Remotes:` is
       up-to-date[^remotes]. For R packages you can use `usethis::use_version()` to update the version number and add a new `NEWS.md` section.
...
The release process for CRAN releases requires extra time and attention. Make sure to familiarise yourself with [the steps to release to CRAN](#release-to-cran). A good place to start would be to 
 ...
 - [ ] run `devtools::release()` and follow the prompts. This will run through a bunch of confirmation checks and finally submit the package to CRAN using `devtools::submit_cran()`.
